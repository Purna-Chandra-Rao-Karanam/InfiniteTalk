...
class InfiniteTalkPipeline:
    ...
    def generate_infinitetalk(self,
        ...
        writer = None
        if stream_save_path and self.rank == 0:
            writer = imageio.get_writer(stream_save_path, fps=25, codec='libx264', quality=5, ffmpeg_params=["-crf", "10"])
        ...
        while True:
            ...
            if is_first_clip:
                video_to_save = videos
            else:
                video_to_save = videos[:, :, cur_motion_frames_num:]

            if writer is not None:
                # Streaming mode: write chunk directly to file
                video_chunk = (video_to_save + 1) / 2 # [-1,1] -> [0,1]
                video_chunk = video_chunk.permute(0, 2, 3, 4, 1).squeeze(0).cpu().numpy() # B C T H W -> T H W C
                video_chunk = np.clip(video_chunk * 255, 0, 255).astype(np.uint8)
