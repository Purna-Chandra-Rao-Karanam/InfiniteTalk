...
def save_video_ffmpeg(gen_video_samples, save_path, vocal_audio_list, fps=25, quality=5, high_quality_save=False):
    ...
    if isinstance(gen_video_samples, str):
        # If input is a string, it's a path to an existing video file (streaming mode)
        import shutil
        if gen_video_samples != save_path_tmp:
            shutil.copy(gen_video_samples, save_path_tmp)
    elif high_quality_save:
        cache_video(
    ...
    # crop audio according to video length
    if isinstance(gen_video_samples, str):
         reader = imageio.get_reader(gen_video_samples)
         meta = reader.get_meta_data()
         if 'duration' in meta:
             duration = meta['duration']
         else:
             # Fallback estimation if duration not in metadata
